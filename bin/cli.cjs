...
Uses ${c("cyan", "ByteDance Jiemeng AI (Seedream)")} via ${c("cyan", "Volcano Engine")} for image editing.
`);
}
...
async function getVolcengineApiKey(rl) {
  logStep("2/7", "Setting up Volcano Engine API key...");

  const VOLCENGINE_URL = "https://console.volcengine.com/";

  log(`\nTo use Jiemeng AI (Seedream), you need a Volcano Engine API key.`);
  log(`${c("cyan", "→")} Get your key from: ${c("bright", VOLCENGINE_URL)}\n`);

  const openIt = await ask(rl, "Open Volcano Engine console in browser? (Y/n): ");

  if (openIt.toLowerCase() !== "n") {
    logInfo("Opening browser...");
    if (!openBrowser(VOLCENGINE_URL)) {
      logWarn("Could not open browser automatically");
      logInfo(`Please visit: ${VOLCENGINE_URL}`);
    }
  }

  log("");
  const apiKey = await ask(rl, "Enter your VOLCENGINE_API_KEY: ");

  if (!apiKey) {
    logError("VOLCENGINE_API_KEY is required!");
    return null;
  }
...
async function main() {
  ...
  try {
    ...
    // Step 2: Get Volcano Engine API key
    const apiKey = await getVolcengineApiKey(rl);
    if (!apiKey) {
      rl.close();
      process.exit(1);
    }
